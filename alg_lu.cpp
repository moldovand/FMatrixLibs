...
double _alg_lu(int n, matrix a, int *ip)
{
  ...
  double t, u, det;
  vector weight;
  weight = _alg_new_vector(n);    /* weight[0..n-1] の記憶領域確保 */
  det = 0;                   /* 行列式 */
...
EXIT:
  _alg_free_vector(weight);  /* 記憶領域を解放 */
  return det;                /* 戻り値は行列式 */
}
...
double _alg_gauss(int n, matrix a, vector b, vector x)
{
  ...
  if (ip == NULL) _alg_error("記憶領域不足");
  det = _alg_lu(n, a, ip);                   /* LU分解 */
  if (det != 0) _alg_solve(n, a, b, ip, x);  /* LU分解の結果を使って連立方程式を解く */
  free(ip);                                  /* 記憶領域の解放 */
  return det;                                /* 戻り値は行列式 */
}
